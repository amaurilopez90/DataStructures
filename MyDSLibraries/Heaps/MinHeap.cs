...
namespace Heaps {
    ...
    public class MinHeap<T> : IHeap<T> where T : struct, IComparable, IFormattable, IConvertible, IComparable<T>, IEquatable<T> {

        private int size;
        private List<T> items;

        //Constructor
        public MinHeap(int _size = 0)
        {
            this.size = _size;
        ...
        private void HeapifyUp()
        {
            ...
            //If we have a "node" whos parent is greater than it's value, then we want to swap these two
        ...
        private void HeapifyDown()
        {
            ...
            {
                //get the least of the left and right children for this index
                int smallerIndex = this.GetLeftChildIndex(index);
                if (this.HasRightChild(index) && this.Right(index).CompareTo(this.items[smallerIndex]) < 0)
                    smallerIndex = this.GetRightChildIndex(index);

                //If parent is greater than the child than swap them
                if (this.items[index].CompareTo(this.items[smallerIndex]) < 0)
                    break;
                else
